...
const generateArrayWithNumber = (num = 0) =>
...
export const FloatingHeart = () => {
  ...
  const getHearts = React.useCallback(
    (numberHearts = defaultNumberOfHeart): Heart[] =>
  ...
  const handleAnimate = React.useCallback(
    async (heartArr: Heart[]) => {
      const min = 10;
      const max = screen?.width;
      const x = random(min, max);
      const y = 0;
      for (const heart of heartArr) {
        animate(
          `#${heart.id}`,
          {
            opacity: [0.8, 0],
